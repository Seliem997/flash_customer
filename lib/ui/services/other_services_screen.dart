...
class _OtherServicesState extends State<OtherServices> {
  ...
  void loadData() async {
    ...
    ).then((value) => otherServicesProvider.getOtherServices(cityId: requestServicesProvider.cityIdData!.id!));
  }
  ...
  Widget build(BuildContext context) {
    ...
    final RequestServicesProvider requestServicesProvider = Provider.of<RequestServicesProvider>(context);
    return Scaffold(
      appBar: CustomAppBar(
      ...
      body: otherServicesProvider.isLoading
          ...
          ? const Expanded(child: Center(child: TextWidget(text: 'There are No Services')))
          : Padding(
                ...
                child: GridView.builder(
                  ...
                  itemBuilder: (context, index) {
                    return  OtherServicesItem(
                      title: otherServicesProvider.otherServicesList[index].title!,
                      imageName: otherServicesProvider.otherServicesList[index].image!,
                      serviceValue: '${otherServicesProvider.otherServicesList[index].selectedPrice} SR',
                      serviceUnit: otherServicesProvider.otherServicesList[index].costType,
                      onlyValue: otherServicesProvider.otherServicesList[index].costType == "pricable" ? true : false,
                      infoOnPressed: (){
                        showDialog(
                          context: context,
                      ...
                      onTap: (){
                        otherServicesProvider.selectedOtherServiceId = otherServicesProvider.otherServicesList[index].id!;
                        otherServicesProvider.selectedService(
                            index: index);

                      },
                      index: index,
                      service: otherServicesProvider.otherServicesList[index],
                    );
                  },
                  gridDelegate:
              ...
              child: DefaultButton(
                ...
                onPressed: () {
                  if(otherServicesProvider.selectedServiceIndex != null){
                    ...
                    otherServicesProvider.storeInitialOtherServices(
                      context,
                      cityId: requestServicesProvider.cityIdData!.id!,
                      otherServiceId: otherServicesProvider.selectedOtherServiceId,
                      addressId: addressesProvider.addressDetailsData!.id!,
                      numberOfUnits: otherServicesProvider.otherServicesList[otherServicesProvider.selectedServiceIndex!].quantity,
                    ).then((value) {
                      AppLoader.stopLoader();
                      if(value.status == Status.success){
                        navigateTo(context, const SelectDate(cameFromOtherServices: true,));
                      }else{
                        CustomSnackBars.failureSnackBar(context, '${value.message}');
                      }
                    });
                  }else{
                    CustomSnackBars.failureSnackBar(context, 'Please Select Service');
                  }
                },
              ),
            ),
